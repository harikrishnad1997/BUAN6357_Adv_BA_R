...
in1_1 <- in1
in1_1$grp <- factor(in1$grp,levels = 1:3)
if(demo) {str(in1)}
if(demo) {str(in1_1)}
...
```
Adding levels to the grps to make it ordinal in nature

```{r}

t         <- data.table(idx=1:3, i=1:3) 
tLogit     <- t[,.(fitted=fitLogit(in1_1,i)), by=.(idx)]
mLogit    <- matrix(tLogit$fitted, ncol=length(classif),byrow=F) 
idxLogit  <- apply(mLogit,byRows,which.max) 
classLogit<- classif[idxLogit] 
rMargin   <- mLogit[,1]+mLogit[,2]+mLogit[,3] 
...
pLogit    <- t1/rMargin 
brLogit   <- 1-pLogit 
(tbl1_1      <- table(in1_1$grp,classLogit,dnn=c("grp","class")) )
...
pTree     <- apply(mnTree,byRows,max) 
brTree    <- 1-pTree 
(tbl2_1      <- table(in1_1$grp,classTree,dnn=c("grp","class")) )
